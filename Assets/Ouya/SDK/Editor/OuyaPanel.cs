...
public class OuyaPanel : EditorWindow
{
    ...
    {
        string path = "Assets/Plugins/Android/src/OuyaUnityApplication.java";
        FileInfo fi = new FileInfo(path);
        return fi.FullName;
    }
    ...
    void Update()
    {
        ...
        if (m_toggleBuildApplication)
        {
            ...
            BuildPipeline.BuildPlayer(sceneArray, string.Format("{0}/OuyaUnityApplication.apk", pathUnityProject), BuildTarget.Android, BuildOptions.None);
        }
        ...
        if (m_toggleBuildAndRunApplication)
        {
            ...
            BuildPipeline.BuildPlayer(null, string.Format("{0}/OuyaUnityApplication.apk", pathUnityProject), BuildTarget.Android, options);
        }
    ...
    void CompileApplicationClasses()
    {
        ...
        string includeFiles = string.Format("\"{0}/OuyaUnityApplication.java\" \"{1}\"", pathSrc, pathRJava);
        ...
        {
            case RuntimePlatform.OSXEditor:
                jars = string.Format("\"{0}:{1}:{2}:{3}\"", pathToolsJar, GetPathAndroidJar(), pathGsonJar, pathUnityJar);
